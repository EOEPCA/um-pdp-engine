...
class PEPResourceTest(unittest.TestCase):
    ...
    def getResourcePolicyList(self, id_token="filler"):
        headers = { 'content-type': "application/x-www-form-urlencoded", "cache-control": "no-cache", "Authorization": "Bearer "+str(id_token)}
        res = requests.get(self.PDP_HOST+":"+self.PDP_PORT+"/policy/", headers=headers, data={"resource_id": self.resource_id} verify=False)
    ...
    def test_resource(self):
        ...
        #Mock resource ID
        resource_id = "test_resource_id_0002"
        ...
        #Get policy list for the specific mock resource
        status, reply = self.getResourcePolicyList(id_token)
        self.assertEqual(status, 200)
        ...
        found = False
        for r in reply:
            if r["_id"] == self.policyID: found = True
        self.assertTrue(found)
        print("Get policy list: Policy found on Internal List.")
        print(reply)
        del status, reply
        print("=======================")
        print("")
